...
exports.load = function (uuid, callback) {
  FS.readFile(Path.join(config.worldPath, dirs.stats, uuid + ".json"), function (err, data) {
    ...
    var
    rgx = /^stat\.(mineBlock\.|useItem\.|(deaths|leaveGame|chestOpened|trappedChestTriggered|hopperInspected|dropperInspected|dispenserInspected|furnaceInteraction|brewingstandInteraction|mobKills)$)/,
    res = {};
